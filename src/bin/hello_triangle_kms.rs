...
// KMS headless runner for upstream hello_triangle, reusing our headless path.
// Minimal, self-contained copy of the KMS CPU scanout path so we don't have to
// refactor the main binary. Uses the HelloTriangle example module for rendering.
...
#[path = "../../examples/upstream/triangle_upstream.rs"]
mod triangle_upstream;
use triangle_upstream::HelloTriangle;
...
struct DrmPick { connector_id: u32, width: u32, height: u32, refresh_millihz: u32 }
...
// ----- KMS CPU presenter (dumb buffers + vblank page flips) -----
struct DbBuf { dbuf: ctrl::dumbbuffer::DumbBuffer, fb: ctrl::framebuffer::Handle }
...
impl KmsCpuPresenter {
    ...
    fn copy_to_readback(&self, encoder: &mut wgpu::CommandEncoder) {
        encoder.copy_texture_to_buffer(
            ...
            wgpu::TexelCopyBufferInfo {
                ...
                layout: wgpu::TexelCopyBufferLayout { offset: 0, bytes_per_row: Some(self.copy_bpr), rows_per_image: Some(self.height) },
            },
    ...
    fn begin_frame(&self, device: &wgpu::Device) -> wgpu::CommandEncoder {
        device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("encoder") })
    }

    fn end_frame(&mut self, mut encoder: wgpu::CommandEncoder, device: &wgpu::Device, queue: &wgpu::Queue) -> Result<()> {
...
fn main() -> Result<()> {
        ...
        .context("No suitable adapter for offscreen KMS scanout")?;
    {
        ...
        log::info!("hello_triangle_kms using adapter: name='{}' backend={:?} type={:?}", info.name, info.backend, info.device_type);
    }
    ...
    let mut presenter = KmsCpuPresenter::new(card, pick, &device).context("init KMS CPU presenter")?;

    let mut app = HelloTriangle::new(
        &device,
        &queue,
        presenter.preferred_format(),
        triangle_upstream::ExampleSize { width: presenter.width, height: presenter.height },
    )?;
    ...
    loop {
        ...
        last = now;
        let mut encoder = presenter.begin_frame(&device);
        app.render(&mut encoder, &presenter.target_view)?;
        presenter.end_frame(encoder, &device, &queue)?;
    }
}
